...
- **nullable / non-null(`!`) 과 null 전파** — nullable은 "값이 비어 있어도 됨", non-null(타입 뒤 `!`)은 "절대 비면 안 됨"이라는 약속. non-null 칸이 비면 그 빈자리가 부모 객체까지 통째로 비우며 위로 번지는데(null 전파), 그래서 죽을 수 있는 조인 필드는 일부러 nullable로 둔다.
- **Relay cursor connection (커서 페이징)** — "몇 페이지"가 아니라 "이전에 본 마지막 항목 다음부터" 가져오는 목록 방식. 책갈피를 꽂아 거기서부터 이어 읽기라, 데이터가 계속 쌓여도 항목이 밀리거나 중복되지 않는다.
- **opaque cursor (불투명 커서)** — 다음 위치를 가리키는 책갈피 값을 클라이언트가 못 읽게 일부러 가려 둔 것. 안에 "20번째부터"가 들었어도 그냥 다음 요청에 돌려주기만 하면 되니, 나중에 내부 방식을 바꿔도 클라이언트는 영향이 없다.
- **keyset 페이징** — offset(앞에서 몇 번째)을 세지 않고 "마지막으로 본 값보다 큰 것"으로 다음 묶음을 집는 더 빠른 페이징 방식. 뒷페이지로 갈수록 느려지는 offset의 약점을 피한다.
