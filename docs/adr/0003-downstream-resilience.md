...
- **Resilience4j** — 외부 호출이 느려지거나 죽었을 때 우리 서버가 같이 쓰러지지 않게 막아 주는 자바용 "안전장치 모음" 라이브러리. 타임아웃·재시도·차단기 같은 부품을 호출 둘레에 겹겹이 감싼다.
- **서킷 브레이커 (Circuit Breaker) / half-open** — 외부 호출이 자꾸 실패하면 두꺼비집처럼 잠시 회선을 끊어 즉시 실패시키고, 죽은 서버를 계속 두드리지 않게 하는 장치. 일정 시간 뒤 "반쯤 열어(half-open)" 시험 호출 몇 개만 흘려 보고, 멀쩡하면 다시 정상 연결한다.
- **지수 backoff (exponential backoff)** — 재시도 간격을 1초→2초→4초처럼 점점 두 배로 늘리는 것. 넘어진 서버에 모두 동시에 다시 달려들어 또 깔리는 걸 막으려고 간격을 벌린다.
- **부분 실패 격리 (partial failure)** — 한 쿼리가 여러 서버를 부르는데 그중 한 곳이 죽어도 그 칸만 비워(null) 두고 나머지는 정상으로 돌려주는 것. 뷔페에서 한 코너가 비어도 식사 전체를 망치지 않는 셈.
