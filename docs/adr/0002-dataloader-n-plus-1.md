...
- **DataLoader / batch(일괄) 조회** — 조회 요청을 건마다 바로 보내지 않고 아주 짧은 순간 키들을 모았다가 "한 방에" 모아 조회하는 도구. 택배를 한 건씩 부치는 대신 한 트럭에 몰아 싣는 것.
- **resolver(리졸버)** — GraphQL 응답에서 한 필드의 값을 실제로 채워 오는 함수. "이 주문의 인보이스는 뭐야?"라는 칸을 받아 답을 적어 넣는 담당자.
- **병렬 fan-out (코루틴 병렬 호출)** — downstream이 여러 건을 한 번에 받는 창구를 안 열어줄 때, 단건 호출들을 하나씩 차례로가 아니라 동시에 쫙 펼쳐 보내고 답을 모으는 것. 단톡방에 한꺼번에 물어 답을 모으는 셈.
- **coroutine(코루틴) / CompletableFuture(미래값)** — 결과를 기다리는 동안 스레드를 붙잡지 않고 잠깐 양보했다가 답이 오면 이어 달리는 가벼운 비동기 방식. CompletableFuture는 "값은 지금 없지만 나중에 채워질 자리"를 미리 받아 두는 약속표.
