...
- **JWT(JSON Web Token) / 클레임(claim)** — 로그인한 사람의 신분증 역할을 하는, 위조 못 하게 서명이 박힌 토큰. 그 안에 든 "이 사람 id·역할·소속 회사" 같은 정보 한 조각씩을 클레임이라 한다.
- **JWK Set / resource server(리소스 서버)** — JWK Set은 토큰 서명이 진짜인지 확인할 "공개 열쇠 꾸러미"다. 리소스 서버는 토큰을 직접 발급하지 않고 이 열쇠로 서명만 검문하는 쪽(발급은 auth-service만, 게이트웨이·downstream은 검문만).
- **token relay(토큰 릴레이)** — 클라이언트가 보낸 JWT를 게이트웨이가 새로 만들지 않고 받은 그대로 downstream 호출에 넘기는 것. 계주에서 바통을 그대로 다음 주자에게 넘기듯, 신분증 원본을 그대로 전달해 인증 주체를 한 곳으로 유지한다.
- **Reactor Context vs ThreadLocal** — 요청에 딸린 토큰을 옮기는 통로 두 가지. ThreadLocal은 "한 스레드에 붙인 메모"라 비동기로 스레드가 바뀌면 사라지고, Reactor Context는 요청 전체를 따라다녀 스레드를 갈아타도 토큰이 끝까지 살아남는다.
